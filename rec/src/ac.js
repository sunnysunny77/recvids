...
class Ac extends React.Component {
    ...
    tab = (res) => {
        let mm = res.map((key, index) => {
            ...
            return (
                <ul key={index} style={{ width: '275px', listStyleType: "square", marginLeft: "auto", marginRight: "auto" }}>
                    <li className="iii" onClick={() => ReactDOM.render(<Cart vidy={key} uses={this.state.a} />, document.getElementById('root'))} style={{ width: '275px', textAlign: "right" }}>{ss}</li>
                    <br />
                </ul>
            )
        })
    ...
    bout = (v) => {
        ...
        axios.post(`https://rec.videos.sunnytemp.live/do`, { token: token })
            .then(res => {
                let use = jwt.verify(res.data.tokenres, process.env.REACT_APP_JWT_SECRET)
                if (use.message) {
                    ReactDOM.render(<Index msg={use.message} />, document.getElementById('root'))
                } else if (use.message0) {
                    ReactDOM.render(<Index msg={use.message0} />, document.getElementById('root'))
                }
            }).catch(error => {
                alert(error.response.statusText)
            })
        ...
        return (<div className="cont" >
            ...
            <div id="di"    >
                ...
                <div style={{ textAlign: 'center' }} >
                    <Button onClick={this.out} style={{ color: "white", backgroundColor: "black", width: '200px', borderRadius: 0 }} >Log Out</Button>
                </div>
                <br />
                <br />
                ...
                <br />
                <br />
                <div style={{ backgroundColor: 'black', opacity: 0.9, width: '50%', marginLeft: 'auto', minHeight: '100px', minWidth: '400px', marginRight: 'auto', border: 'solid 0.5px white' }} id="ffff">
                </div>
                <br />
                <br />
                <br />
                ...
                <br />
                <br />
                <p id='name' style={{ color: 'white', margin: 0, fontSize: '130%', fontWeight: 700, textShadow: "-2px -2px 0 #000, 2px -2px 0 #000, -2px 2px 0 #000, 2px 2px 0 #000", height: "60px", textAlign: 'center', padding: 0 }}></p>
                <h3 style={{ textAlign: 'center', padding: 0 }}> {this.state.head} </h3>
                <br />
                <div id="paypal-button-container" style={{ textAlign: 'center' }}></div>
                <br />
                <br />
                <div style={{ textAlign: 'center' }} >
                    <p style={{ color: "white", position: "relative", left: "6.5px" }}>&nbsp;&nbsp;Delete Accout - <ExitToAppIcon style={{ position: "relative", top: "6.5px" }} id="ex" onClick={this.bout} /></p>
                </div>
            </div>
