...
class Se extends React.Component {
    ...
    change = (event) => {
        ...
        this.setState({ [nam]: val, b: null, dis: { display: "none" }, c: null, dis1: { display: "none" } })
    }
    file = (event) => {
        this.setState({ p: event.target.files[0].name, file: event.target.files[0], b: null, dis: { display: "none" }, c: null, dis1: { display: "none" } })
    }
    sub0 = (event) => {
        ...
        if (this.state.del) {
            ...
            if (extt === 'mp4') {
                ...
                this.setState({ c: "Delete is not a .mp4", dis1: { display: "block" } })
            }
        } else {
            this.setState({ c: "Delete form imcomplete", dis1: { display: "block" } })
        }
                ...
                if (!endsWith(this.state.file.name.toLowerCase(), ".mp4.mp4")) {
                    if (this.state.file.size < 100000000) {
                        ...
                        if (extension === 'mp4') {
                            ...
                            document.querySelector('.custom-file-upload').style.cursor = 'progress'
                            ...
                            this.setState({ b: "Upload is not a .mp4", dis: { display: "block" } })
                        }
                    } else {
                        this.setState({ b: "Upload is over 100MB", dis: { display: "block" } })
                    }
                } else {
                    this.setState({ b: "Anything but .mp4.mp4", dis: { display: "block" } })
                }
            } else {
    ...
    render() {
        return (<div className="cont">
            ...
            <div id="di">
                ...
                <Tooltip placement="top" title={<span style={{ fontSize: "150%" }}>Upload a .mp4 100MB cap</span>}>
                    <form id="f0" onSubmit={this.sub}>
                        <label htmlFor="file-upload" className="custom-file-upload">
                            <p>{this.state.p}</p>
                        </label>
                        <input id="file-upload" type="file" name="file" onChange={this.file} />
                        <Button id="wy" style={{ color: "white", backgroundColor: "black", width: '100%', height: '60px', fontSize: "120%", borderRadius: 0 }} variant="contained" type="submit" > Upload</Button>
                    </form>
                </Tooltip>
                <br />
                <div style={{ height: "25px" }}>
                    <h3 className="mss" style={this.state.dis}>{this.state.b}</h3>
                </div>
                <br />
                <br />
                <Tooltip placement="top" title={<span style={{ fontSize: "150%" }}>Delete a .mp4</span>}>
                    <form id="f1" onSubmit={this.sub0}>
                        <input maxLength="40" id="del" type="text" name="del" placeholder="Viedo name" onChange={this.change} />
                        <Button id="wy1" style={{ color: "white", backgroundColor: "black", width: '100%', height: '60px', fontSize: "120%", borderRadius: 0 }} variant="contained" type="submit" > Delete</Button>
                    </form>
                </Tooltip>
                <br />
                <div style={{ height: "25px" }}>
                    <h3 className="mss" style={this.state.dis1}>{this.state.c}</h3>
                </div>
            </div>
        </ div>
        )
