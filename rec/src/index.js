    ...
    componentDidMount() {
        axios.post(`https://rec.videos.sunnytemp.live/tok`)
            .then(res => {
                ...
                if (use.user) {
                    ReactDOM.render(<Cart uses={use.user} />, document.getElementById('root'))
                } else if (use.message) {
                    if (this.state.msg) {
    ...
    sub = (event) => {
        ...
        if (!regex.test(this.state.username)) {
            if (this.state.password && this.state.username) {
                ...
                this.setState({ username: null, password: null, a: "Sing Up imcomplete", dis: { display: 'block' } })
            }
        } else {
            this.setState({ username: null, password: null, a: "No special characters", dis: { display: 'block' } })
        }
        ...
        return (<div className="cont">
            <div id="di">
                ...
                <div style={this.state.dis}>
                    <div id="l1" style={{ margin: "auto", marginBottom: "45px", width: '175px', height: '115px' }}>
                        <h1>Rec Videos</h1>
                    </div>
                </div>
                <div id="lo" style={this.state.dis}>
                    <Tooltip placement="top" title={<span style={{ fontSize: "150%" }}>Toggle switch Log In / Sign Up</span>}>
                        <h2 ><AccountCircle style={{ position: "relative", top: '4px', left: '4px' }} />
                            <Switch
                    ...
                    <form id='a2' style={this.state.dis0} onSubmit={this.sub0}>
                        <TextField
                            ...
                            }} variant="filled" type="text" name="username" placeholder='Username:' onChange={this.change} />
                        <TextField
                            inputProps={{
                                maxLength: 16,
                            }}
                            ...
                            }} variant="filled" type="password" name="password" placeholder='Password:' onChange={this.change} />
                        <Button style={{ color: "white", backgroundColor: "black", width: '100%', height: '60px', fontSize: "120%", borderRadius: 0 }} variant="contained" type="submit" > Log In</Button>
                    </form>
                    <form id='a1' style={this.state.dis1} onSubmit={this.sub}>
                        ...
                        <Button style={{ color: "white", backgroundColor: "black", width: '100%', height: '60px', fontSize: "120%", borderRadius: 0 }} variant="contained" type="submit" > Sign Up</Button>
                    </form>
                </div>
            </div>
        </div>
